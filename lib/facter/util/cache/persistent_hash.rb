...
class Facter::Util::Cache::PersistentHash
  ...
  def initialize(cachedir)
    ...
    # TODO: need more protection around this possibly
    @data = {}
  ...
  # performed using a shared lock for thread safety.
  def [](key)
    Facter.debug("Facter::Util::Cache::PersistentHash.[](#{key})")

    sync(:SH) do
      # TODO: first check if the file has changed, if it has lets grab
      #       that data. Otherwise - fall back to file cache
      begin
        mtime = File.stat(cache_file(key)).mtime.to_i
      rescue Errno::ENOENT
        mtime = nil
      end

      unless entry = @data[key]
        return nil
      end

      if entry \
        and entry.include?("file_mtime") \
        and entry["file_mtime"].eql?(mtime) then

        Facter.debug("Facter::Util::Cache::PersistentHash.[](#{key}): data hasn't changed. File mtime is #{mtime}. Stored mtime is #{entry["file_mtime"]}")

        # Data hasn't changed return existing data
        return entry
      else
        Facter.debug("Facter::Util::Cache::PersistentHash.[](#{key}): data has changed. mtime is #{mtime}. stored mtime is #{entry.inspect}")

        val = load_entry(key)

        # If the returned value differs from what is in our hash, update
        # the hash
        if entry.eql?(val) then
          Facter.debug("Facter::Util::Cache::PersistentHash.[](#{key}): memory and file value match for #{key}.")
          return val
        else
          Facter.debug("Facter::Util::Cache::PersistentHash.[](#{key}): file entry #{val.inspect} mem entry #{entry.inspect} differ - writing cache")
          send(:'[]=', key, val)
          return val
        end
      end
