# Fact: zones_*
#
# Purpose: Return the list of zones on the system and add one zones_ fact
# for each zone with its state e.g. running, incomplete or installed.
#
# Resolution:
#   Uses 'usr/sbin/zoneadm list -cp' to get the list of zones in separate
#   parsable lines with delimeter being ':' which is used to split the line
#   string and get the zone details.
#
# Caveats:
...
Facter.add("zones") do
  ...
  zones_list.each_line do |thisline|
    num_zones += 1
    # add a zone_[name] fact with its status for each zone in list
    Facter.add("zone_#{thisline.split(':')[1]}") do
      setcode do
        thisline.split(":")[2]
      end
    end
